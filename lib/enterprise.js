...
const { SystemTypes } = require('./system/systemTypes.js');
...
class Enterprise {
  constructor(application) {
    ...
    this.systemTypes = new SystemTypes(this); // Определения типов
    this.metadata.systemTypes = this.systemTypes.metadata;
  ...
  async loadMetadata() {
    await this.systemActions.init();
    await this.systemData.init();
  }
  ...
  async loadMetadataFromFiles(pathDir) {
    ...
    await Promise.all(Array.from(files).map(file => new Promise(async resolve => {
      ...
      if (file.isDirectory()) {
        ...
        if (file.name.endsWith('.js')) {
          ...
          if (typeMetadata[0] !== 'me#') return resolve();
    ...
    for (const field of this.preloadSchemasData) {
      field.bindForeignKey();
    };
