...
class EStatic {
  ...
  get(key) {
    if (!this.files.has(key)) return this.files.get('/index.html');
  ...
  startWatch() {
    ...
    this.watcher.on('change', (filePath) => {
      ...
      node.fs.stat(filePath, (err, stat) => {
        ...
        if (this.application.sandbox.application.worker.id === 'W1') {
          this.application.console.debug('Reload: ' + filePath);
        }
    ...
    this.watcher.on('delete', async (filePath) => {
        ...
        if (this.application.sandbox.application.worker.id === 'W1') {
          this.application.console.debug('Deleted: ' + filePath);
        }
    });
  }
